...
for model in models:
    ...
    for i in range(eval_months):
        ...
        # Crea una lista sulla quale fare la predizione a ogni ciclo
        predict_data = shampoo_sales[cutoff_month+i-3-1:cutoff_month+i-1]
        print(predict_data)
        # Utilizza il modulo della classe per fare la previsione sulla lista
        prediction = model.predict(predict_data)

        # Salva il valore reale
        real = shampoo_sales[cutoff_month+i]

        # Stampa i valori predetti in confronto a quelli veri
